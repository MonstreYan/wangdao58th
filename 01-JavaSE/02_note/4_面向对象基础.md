...
- 类是什么东西，对象是什么东西。 （<span style=color:yellow;background:red>**对象与类**</span>）
- 引用数据类型
- 局部变量和成员变量
- this也是个重点。 
- static
- 代码块里面的静态代码块，是个重点。 
- 导包会用。 全限定类名、网上拷的代码出错了，怎么解决。 
- 访问权限修饰符。 
...
<span style=color:yellow;background:red>**一定要知道是怎么输出的。**</span>
...
今后，如果出现，从网上找到的代码。包位置报错。 直接将包声明删掉。 然后在类名报错的地方，点击 alt+enter.让idea自动完成包名的设置。 
...
注意： 后续你见到别人写。 知道这个是啥就可以。 我们不推荐大家这样使用。 
...
2. 缺省(默认): 同一包中的子类或者其它类能够访问，同包中都可以使用。不同包，不能使用。 
